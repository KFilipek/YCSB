...
class Retriever implements ValueCallback<byte[]> {
  @Override
  public void process(byte[] byteBuffer) {
    bb = Arrays.copyOf(byteBuffer, byteBuffer.length);
  }
...
public class PmemKV extends DB {
  ...
  public Status read(final String table, final String key, final Set<String> fields,
    ...
    try {
      db.get(key.getBytes(), retriever);
    } catch (NotFoundException e) {
      return Status.NOT_FOUND;
    }
    if (retriever.getArray() == null) {
      return Status.NOT_FOUND;
    }
    deserializeValues(retriever.getArray(), fields, result);
    return Status.OK;
  }
